function PieCharts() {
    ...
    this.updatePopulationPieChart = function() {
        const demographics = simulation.getDemographics();

        rect(this.ctx, 0, 0, WIDTH/2, HEIGHT, "#ffffff") // Clear background
        text(this.ctx, 'Population', POPULATION_X, 25, '#000', 15);
        this.drawPieSlice(BLUE, POPULATION_X, demographics, GRID_WIDTH**2);
        this.drawPieSlice(RED, POPULATION_X, demographics, GRID_WIDTH**2);
    ...
    window.addEventListener('mousemove', function(event) {
        if (pieCharts.populationDragging) {
    ...
    this.canvas.addEventListener('mousedown', function(event) {
        if (pieCharts.populationHovering) {
            pieCharts.populationDragging = true;
            document.body.style.cursor = 'grabbing';
        }
    });

    this.canvas.addEventListener('mouseup', function(event) {
        pieCharts.populationDragging = false;
        document.body.style.cursor = 'default';
    });

    /** Returns a point in canvas coords that is a point to drag on the population chart */
    this.getPopulationDragPoint = function() {
        const percent = (simulation.getDemographics().get(BLUE) / NUM_PEOPLE);
